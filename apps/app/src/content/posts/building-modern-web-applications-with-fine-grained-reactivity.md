...
function ShoppingCart() {
	...
	const itemCount = createMemo(() => items().reduce((count, item) => count + item.quantity, 0));
...
function ProductCard(props) {
	...
	const totalPrice = createMemo(() => props.product.price * quantity());
	...
	return (
		<div class="product-card">
			...
			<button class={isFavorite() ? "favorite active" : "favorite"} onClick={toggleFavorite}>
				{isFavorite() ? "❤️" : "🤍"}
			</button>
		</div>
	);
		...
		<table>
					...
					<For each={columns}>
						{(column) => (
							<th onClick={() => handleSort(column.key)}>
								{column.name}
								{sortBy() === column.key && <span>{sortDirection() === "asc" ? "↑" : "↓"}</span>}
							</th>
						)}
					</For>
				</tr>
			...
			<tbody>
				<For each={sortedData()}>
					{(item) => (
						<tr>
							<For each={columns}>{(column) => <td>{item[column.key]}</td>}</For>
						</tr>
					)}
				</For>
			</tbody>
